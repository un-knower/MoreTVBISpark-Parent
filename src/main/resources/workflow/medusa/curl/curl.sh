...
##获取sessionid
echo "================getssionid====================="
echo 'getssionid : curl -k -X POST --data "action=login&username=$username&password=$password" ' $weburl
...
echo "execution flow : curl -k --get --data 'session.id='$sessionid --data 'ajax=executeFlow' --data 'project='$project --data 'flow='$flow --data \"failureAction=finishPossible\" --data \"notifyFailureFirst=false\" ${args} $weburl/executor"
exeCommand="curl -k --get --data \"session.id=$sessionid\" --data \"ajax=executeFlow\" --data \"project=$project\" --data \"flow=$flow\" --data \"failureAction=finishPossible\" --data \"notifyFailureFirst=false\" ${args} $weburl/executor "
