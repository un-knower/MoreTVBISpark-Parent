...
/**
  ...
  *
  */
object RecordChannelClassificationStat extends BaseClass {
  private val tableName = "medusa_channel_eachtab_play_jilu_info"
  ...
  private val analyse_source_data_df_name = "jilu_channel_classification_analyse_source_data_df"
  private val analyse_result_df_name = "jilu_channel_classification_analyse_result_df"
      ...
      case Some(p) => {
        ...
        (0 until p.numOfDays).foreach(w => {
          ...
          if (medusaFlag && moretvFlag) {
            ...
            sqlStr =
              s"""
                 ...
                 |from medusa_table
                     """.stripMargin
            println("--------------------" + sqlStr)
            val jilu_medusa_table_step2_df = sqlContext.sql(sqlStr)
            jilu_medusa_table_step2_df.cache()
            jilu_medusa_table_step2_df.registerTempTable("medusa_table_step2")
            writeToHDFSForCheck(date, "jilu_medusa_table_step2_df", jilu_medusa_table_step2_df, p.deleteOld)
            sqlStr =
              s"""
                 |select a.userId,
                 |       a.videoSid,
            ...
            sqlStr =
              s"""
                 ...
                 |from moretv_table
                     """.stripMargin
            println("--------------------" + sqlStr)
            val jilu_moretv_table_step2_df = sqlContext.sql(sqlStr)
            jilu_moretv_table_step2_df.cache()
            jilu_moretv_table_step2_df.registerTempTable("jilu_moretv_table_step2")
            writeToHDFSForCheck(date, "jilu_moretv_table_step2_df", jilu_moretv_table_step2_df, p.deleteOld)
            sqlStr =
              s"""
                 |select a.userId,
                 |       a.videoSid,
                 ...
                 |       b.second_category,
                 |       a.flag
                 |from jilu_moretv_table_step2                       a
                 |join
                 |    ${DimensionTypes.DIM_MEDUSA_SOURCE_SITE}  b
            ...
            mergerRDD.cache()
            val jilu_step1_table_df = sqlContext.read.json(mergerRDD)
            jilu_step1_table_df.cache()
            jilu_step1_table_df.registerTempTable("jilu_step1_table")
            writeToHDFSForCheck(date, "cc_jilu_step1_table_df", jilu_step1_table_df, p.deleteOld)

            /** step2 用于过滤单个用户播放当个视频量过大的情况 */
            sqlStr =
              s"""
                 |select concat(userId,videoSid) as filterColumn
                 |from jilu_step1_table
