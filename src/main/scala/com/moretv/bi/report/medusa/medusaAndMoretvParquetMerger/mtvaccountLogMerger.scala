...
/**
  ...
  */
object mtvaccountLogMerger extends BaseClass{
       ...
       case Some(p)=>{
        /* val logType = "mtvaccount"
         ...
         /*  val logDir1 = s"$medusaDir/$inputDate/$logType"
           ...
           val medusa_input_dir= DataIO.getDataFrameOps.getPath(MEDUSA,LogTypes.MTVACCOUNT,inputDate)
           val moretv_input_dir= DataIO.getDataFrameOps.getPath(MORETV,LogTypes.MTVACCOUNT,inputDate)
           val outputPath= DataIO.getDataFrameOps.getPath(MERGER,LogTypes.MTVACCOUNT,inputDate)
          ...
          if(p.deleteOld){
           ...
           if(medusaFlag && moretvFlag){
            /* val medusaDf = sqlContext.read.parquet(logDir1)
             ...
             val medusaDf = DataIO.getDataFrameOps.getDF(sqlContext,p.paramMap,MEDUSA,LogTypes.MTVACCOUNT,inputDate)
             val moretvDf = DataIO.getDataFrameOps.getDF(sqlContext,p.paramMap,MORETV,LogTypes.MTVACCOUNT,inputDate)
