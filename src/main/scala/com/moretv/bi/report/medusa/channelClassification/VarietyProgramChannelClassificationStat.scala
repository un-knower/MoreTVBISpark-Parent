...
/**
  ...
  *
  */
object VarietyProgramChannelClassificationStat extends BaseClass {
        ...
        (0 until p.numOfDays).foreach(w => {
          ...
          sqlStr =
            s"""
               ...
               |      main_category='$CHANNEL_VARIETY_PROGRAM'
          ...
          mysql_result_df.collect.foreach(row => {
            util.insert(sqlInsert, sqlDate, CHANNEL_VARIETY_PROGRAM, row.getString(0), new JLong(row.getLong(1)), new JLong(row.getLong(2)))
          })
        })
      }
      case None => {
