      ...
      /*   val logType = "enter"
         ...
         (0 until p.numOfDays).foreach(i=>{
          ...
          /* val logDir1 = s"$medusaDir/$inputDate/$logType"
           ...
           val medusa_input_dir= DataIO.getDataFrameOps.getPath(MEDUSA,LogTypes.ENTER,inputDate)
           val moretv_input_dir= DataIO.getDataFrameOps.getPath(MORETV,LogTypes.ENTER,inputDate)
           val outputPath= DataIO.getDataFrameOps.getPath(MERGER,LogTypes.ENTER,inputDate)
           ...
           if(medusaFlag && moretvFlag){
            /* val medusaDf = sqlContext.read.parquet(logDir1)
             ...
             val medusaDf = DataIO.getDataFrameOps.getDF(sqlContext,p.paramMap,MEDUSA,LogTypes.ENTER,inputDate)
             val moretvDf = DataIO.getDataFrameOps.getDF(sqlContext,p.paramMap,MORETV,LogTypes.ENTER,inputDate)
