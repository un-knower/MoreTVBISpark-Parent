...
/**
  ...
  */
object appRecommendInstallInfo extends BaseClass{
    ...
    ParamsParseUtil.parse(args) match {
      case Some(p) => {
        ...
        (0 until p.numOfDays).foreach(i=>{
          ...
          if(p.deleteOld){
            val deleteSql="delete from medusa_app_recommend_install_info where day=?"
            util.delete(deleteSql,insertDate)
          }


          val sqlInsert = "insert into medusa_app_recommend_install_info(day,appSid,appName,user_num,play_num) " +
            "values (?,?,?,?,?)"
