...
/**
  ...
  *
  */
object TvChannelClassificationStat extends BaseClass {
  private val tableName = "medusa_channel_eachtab_play_tv_info"
      ...
      case Some(p) => {
        ...
        (0 until p.numOfDays).foreach(w => {
          ...
          if (medusaFlag && moretvFlag) {
            ...
            sqlStr =
              s"""
                 ...
                 |from medusa_table
                     """.stripMargin
            println("--------------------" + sqlStr)
            val tv_medusa_table_step2_df = sqlContext.sql(sqlStr)
            tv_medusa_table_step2_df.cache()
            tv_medusa_table_step2_df.registerTempTable("medusa_table_step2")
            writeToHDFSForCheck(date, "tv_medusa_table_step2_df", tv_medusa_table_step2_df, p.deleteOld)
            sqlStr =
              s"""
                 |select a.userId,
                 |       a.videoSid,
            ...
            sqlStr =
              s"""
                 ...
                 |from moretv_table
                     """.stripMargin
            println("--------------------" + sqlStr)
            val tv_moretv_table_step2_df = sqlContext.sql(sqlStr)
            tv_moretv_table_step2_df.cache()
            tv_moretv_table_step2_df.registerTempTable("tv_moretv_table_step2")
            writeToHDFSForCheck(date, "tv_moretv_table_step2_df", tv_moretv_table_step2_df, p.deleteOld)
            sqlStr =
              s"""
                 |select a.userId,
                 |       a.videoSid,
            ...
            mergerRDD.cache()
            val tv_step1_table_df = sqlContext.read.json(mergerRDD)
            tv_step1_table_df.cache()
            tv_step1_table_df.registerTempTable("tv_step1_table")
            writeToHDFSForCheck(date, "cc_tv_step1_table_df", tv_step1_table_df, p.deleteOld)
          ...
          mysql_result_df.collect.foreach(row => {
            util.insert(sqlInsert, sqlDate, CHANNEL_TV, row.getString(0), new JLong(row.getLong(1)), new JLong(row.getLong(2)))
          })
        })
      }
      case None => {
