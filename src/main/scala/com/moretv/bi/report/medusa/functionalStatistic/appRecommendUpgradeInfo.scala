...
object appRecommendUpgradeInfo extends BaseClass{
  val tableName = "medusa_app_recommend_upgrade_info"
    ...
    ParamsParseUtil.parse(args) match {
      case Some(p) => {
        ...
        (0 until p.numOfDays).foreach(i=>{
          ...
          val rdd = df.sqlContext.sql(
            "select a.appSid, count(distinct a.userId), count(a.userId), first(b.application_name) from log_data a "
              + " left join dim_application b on a.appSid = b.application_sid and b.dim_invalid_time is null"
              + s" where a.event='upgrade' group by a.appSid "
