package com.moretv.bi.report.medusa.userDevelop

import cn.whaley.sdk.dataexchangeio.DataIO
import com.moretv.bi.global.{DataBases, LogTypes}
import com.moretv.bi.report.medusa.channeAndPrograma.mv.af310.SingerPlayStat._
import com.moretv.bi.report.medusa.util.PeriodJob
import com.moretv.bi.util.ParamsParseUtil
import com.moretv.bi.util.baseclasee.{BaseClass, ModuleClass}
import org.apache.spark.sql.DataFrame
import org.joda.time.LocalDate
import org.joda.time.format.DateTimeFormat

import scala.collection.mutable.ListBuffer

/**
  * Created by witnes on 3/29/17.
  */


object PeriodUserDevelopStat extends PeriodJob {

  pathPatterns = LOGINLOG :: DBSNAPSHOT :: Nil

  logTypes = LogTypes.MORETV_MTV_ACCOUNT :: LogTypes.MORETV_MTV_ACCOUNT :: Nil


  tbl = "user_develop_period_stat"

  fields = Array(
    "execute_day",
    "period_type",
    "period_value",
    "new_user_count",
    "login_user_count",
    "login_count"
  )


  override def periodAggHandle(periodType: String, periodValue: String, givenDate: LocalDate): Unit = {

    val metrics = ListBuffer[Long]()

    (logTypes zip pathPatterns).foreach {

      case (logType, pathPattern) => {

        pathPattern match {

          case DBSNAPSHOT => {

            val res2 = aggregateNewUserStat(
              getPeriodSnapshot(periodType, 1, givenDate, logType, pathPattern)
            )

            metrics.append(res2)
          }

          case LOGINLOG => {

            val res1 = aggregateLoginUserStat(
              getPeriodPartition(periodType, 1, givenDate, logType, pathPattern)
            )
            metrics.append(res1._1, res1._2)

          }
        }
      }
    }

    val exporter = DataIO.getMySqlOps(DataBases.MORETV_MEDUSA_MYSQL)

    val li = metrics.toList

    if (isDeleteOld) {
      exporter.delete(deleteSql, givenDate.toString("yyyy-MM-dd"), periodType, periodValue)
    }

    exporter.insert(insertSql,
      givenDate.toString("yyyy-MM-dd"),
      periodType, periodValue, li(2), li(0), li(1)
    )

  }

  def aggregateNewUserStat(ods: DataFrame): Long = {

    val sq = sqlContext
    import sq.implicits._
    import org.apache.spark.sql.functions._

    ods.agg(
      countDistinct($"user_id")
    )
      .first()
      .getLong(0)
  }


  def aggregateLoginUserStat(ods: DataFrame): (Long, Long) = {

    val sq = sqlContext
    import sq.implicits._
    import org.apache.spark.sql.functions._

    val row = ods.agg(
      countDistinct($"userId"), count($"userId")
    )
      .first()

    (row.getLong(0), row.getLong(1))

  }

}
