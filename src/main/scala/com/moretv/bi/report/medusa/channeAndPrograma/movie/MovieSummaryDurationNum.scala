...
object MovieSummaryDurationNum extends BaseClass{
  ...
  override def execute(args: Array[String]) {
     ...
     val dbMode = "medusa"
     val tableName = "medusa_programa_movie_summary_view_duration"
     val sqlSpark = "select sum(duration),count(distinct userId) from log_data where event='exit' and " +
       "contentType='movie'"
     val sqlInsert = "insert into medusa_programa_movie_summary_view_duration(day,view_duration,user_num) values (?,?,?)"
