...
/**
  ...
  *
  */
object HotChannelClassificationStat extends BaseClass {
  private val tableName = "medusa_channel_eachtab_play_hot_info"
  ...
  private val analyse_source_data_df_name = "hot_channel_classification_analyse_source_data_df"
  private val analyse_result_df_name = "hot_channel_classification_analyse_result_df"
      ...
      case Some(p) => {
        ...
        (0 until p.numOfDays).foreach(w => {
          ...
          if (medusaFlag && moretvFlag) {
            ...
            sqlStr =
              s"""
                 ...
                 |from medusa_table
                     """.stripMargin
            println("--------------------" + sqlStr)
            val hot_medusa_table_step2_df = sqlContext.sql(sqlStr)
            hot_medusa_table_step2_df.cache()
            hot_medusa_table_step2_df.registerTempTable("medusa_table_step2")
            writeToHDFSForCheck(date, "hot_medusa_table_step2_df", hot_medusa_table_step2_df, p.deleteOld)
            sqlStr =
              s"""
                 |select a.userId,
                 |       a.videoSid,
            ...
            sqlStr =
              s"""
                 ...
                 |from moretv_table
                     """.stripMargin
            println("--------------------" + sqlStr)
            val hot_moretv_table_step2_df = sqlContext.sql(sqlStr)
            hot_moretv_table_step2_df.cache()
            hot_moretv_table_step2_df.registerTempTable("hot_moretv_table_step2")
            writeToHDFSForCheck(date, "hot_moretv_table_step2_df", hot_moretv_table_step2_df, p.deleteOld)
            sqlStr =
              s"""
                 |select a.userId,
                 |       a.videoSid,
            ...
            mergerRDD.cache()
            val hot_step1_table_df = sqlContext.read.json(mergerRDD)
            hot_step1_table_df.cache()
            hot_step1_table_df.registerTempTable("hot_step1_table")
            writeToHDFSForCheck(date, "cc_hot_step1_table_df", hot_step1_table_df, p.deleteOld)

            /** step2 用于过滤单个用户播放当个视频量过大的情况 */
            sqlStr =
              s"""
                 |select concat(userId,videoSid) as filterColumn
                 |from hot_step1_table
          ...
          mysql_result_df.collect.foreach(row => {
            util.insert(sqlInsert, sqlDate, CHANNEL_HOT, row.getString(0), new JLong(row.getLong(1)), new JLong(row.getLong(2)))
          })
        })
      }
      case None => {
