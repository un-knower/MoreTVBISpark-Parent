...
object DataAnalyticsPlayAllThresholdResultDistribution extends BaseClass {

  private val tableName = "data_analytic_all_play_threshold_result_distribution"
    ...
    ParamsParseUtil.parse(args) match {
      case Some(p) => {
        ...
        (0 until p.numOfDays).foreach(i => {
          ...
          val insertSql = s"insert into $tableName(day,version,play_num,valid_play_user,valid_play_num,total_play_user,total_play_num) values(?,?,?,?,?,?,?)"
          //临时没有该parquet文件,容错
          ...
          sqlContext.sql(
            """
              |select getVersion(apkVersion) as version,count(distinct userId) as playUser,count(userId) as playNum
              |from log_data
              ...
              |group by getVersion(apkVersion)
          ...
          playThreshold.collect.foreach(e => {
            sqlContext.sql(
              s"""
                |select version,count(userId) as validPlayUser,sum(playNum) as validPlayNum
                |from log_play
                |where playNum<=${e}
                |group by version
            ...
            sqlContext.sql(
              s"""
                |select a.version,${e} as playNum,a.validPlayUser,a.validPlayNum,
                |b.playUser,b.playNum
                |from log_play_valid as a
                |join log_total_play as b
                |on a.version = b.version
              """.stripMargin).
              map(e=>(e.getString(0),e.getInt(1),e.getLong(2),e.getLong(3),e.getLong(4),e.getLong(5))).
              collect().
              foreach(r=>{
                util.insert(insertSql,insertDate,r._1,r._2,r._3,r._4,r._5,r._6)
              })
          })
