...
import com.moretv.bi.whiteMedusaVersionEstimate.UserUpdateStatistic.{MEDUSA_DIMENSION, sc, sqlContext}

/**
  * Created by QIZHEN on 2017/5/26.
  * 计算所有版本点播主要频道的平均播放时长和播放次数
  *
  */
object videoMainChannelAvgPlayStatisticByVersion extends BaseClass{
  private val tableName = "videoMainChannel_AvgPlayStatistic_byVersion"
  private val insertSql = s"insert into ${tableName}(day,apkVersion,contentType,play_cnt,duration,user_cnt,avg_playCnt,avg_playDuration) values (?,?,?,?,?,?,?,?)"
        ...
        (0 until p.numOfDays).foreach( i => {
          ...
          DataIO.getDataFrameOps.getDF(sc, p.paramMap, MEDUSA, LogTypes.PLAY, date).
           ...
           val updatePlayCnt = sqlContext.sql(
             s"""
                |select a.apkVersion,a.contentType,a.play_cnt,b.duration,a.user_cnt,
               ...
               """.stripMargin).map(e => (e.get(0), e.get(1), e.get(2), e.get(3), e.get(4), e.get(5), e.get(6)))

          if(p.deleteOld) util.delete(deleteSql,insertDate)

          updatePlayCnt.collect.foreach(e => {
            util.insert(insertSql, insertDate,e._1, e._2,e._3,e._4,e._5,e._6,e._7)
          })
