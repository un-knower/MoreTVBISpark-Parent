...
/**
  * 该类统计315电视猫每日活跃用户的留存情况
  */
object Medusa315DAURetentionRate extends BaseClass {
      ...
      case Some(p) => {
        ...
        for (i <- 0 until numOfDays) {
          ...
          for (j <- 0 until needToCalc.length) {
            ...
            if (j == 0) {
              insertSQL(insertDate, "3.1.5", dauNum, retentionRate, stmt1)
            } else {
              updateSQL(numOfDay(j), "3.1.5", retentionRate, insertDate, stmt1)
            }
          }
          logUserID.unpersist()
        }
