...
/**
  ...
  */
object KidsPathParser {

  private val KIDS_3X_RECOMMEND_REGEX = ("(.*)(kids_home|kids)$").r        // 3x少儿频道主页推荐位正则
  private val KIDS_3X_LIST_CATEGORY_REGEX = ("([a-zA-Z0-9_]+)\\*([\\u4e00-\\u9fa5A-Z0-9-]+)").r    // 3x少儿频道站点树正则
  private val KIDS_3X_LIST_CATEGORY_SEARCH_REGEX = ("(.*)-(search)\\*([A-Z]+)$").r                // 3x少儿频道站点树中的搜索正则

  private val KIDS_2X_RECOMMEND_REGEX = ("(.*)(kids_recommend)-(0-9a-zA-Z)$").r    // 2x少儿频道主页推荐位正则
  private val KIDS_2X_LIST_CATEGORY_REGEX = ("(.*)-(kids_home)-([0-9a-z_]+)-(.*)").r                                 // 2x少儿频道站点树正则
