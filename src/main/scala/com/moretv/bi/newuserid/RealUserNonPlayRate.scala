...
/**
  ...
  *
  */
object RealUserNonPlayRate extends BaseClass {
    ...
    ParamsParseUtil.parse(args) match {
      case Some(p) => {
        ...
        (0 until p.numOfDays).foreach(i => {
          ...
          if(p.deleteOld){
            val deleteSql = "delete from real_user_non_play_rate where day = ?"
          ...
          val sqlData = s"select user_id from mtv_account_migration where id >= ? and id <= ? and left(openTime,10) = '$dayCN'"
          ...
          sqlContext.read.load(s"/log/medusa/parquet/$logDates/play").
          ...
          val insertSql = "insert into real_user_non_play_rate(day,new_user,action_user) values(?,?,?)"
