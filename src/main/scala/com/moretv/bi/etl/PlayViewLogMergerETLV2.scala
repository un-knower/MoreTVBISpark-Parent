...
/**
  ...
  *
  */
object PlayViewLogMergerETLV2 extends BaseClass {
        ...
        (0 until p.numOfDays).foreach(i => {
          ...
          if (medusaFlag && moretvFlag) {
            ...
            sqlStr = s"""
                     ...
                     """.stripMargin
            println("--------------------" + sqlStr)
            sqlContext.sql(sqlStr).registerTempTable("medusa_table_init_table_2")

            //过滤列表页中的脏数据[不包含'kids','sports','mv']
            sqlStr =
              s"""
                 |select $medusaColNamesWithTable,
                 |       a.entryType,
                 |       a.main_category,
                 |       if(b.second_category is null,'',b.second_category) as second_category,
                 |       a.third_category,
                 |       a.subjectCode,
                 |       'medusa' as flag
                 |from medusa_table_init_table_2                a
                 |left join
                 |    ${DimensionTypes.DIM_MEDUSA_SOURCE_SITE}  b
                 |    on  a.main_category=b.site_content_type and a.second_category=b.second_category
                 |    and b.main_category_code!='program_site'
                 |where  a.main_category not in ('kids','sports','mv')
                     """.stripMargin

            //过滤列表页中的脏数据[包含'kids','sports','mv']
            sqlStr =
              s"""
                 |select $medusaColNamesWithTable,
                 |       a.entryType,
                 ...
                 |    on  a.main_category=b.site_content_type and a.second_category=b.second_category
                 |    and b.main_category_code!='program_site'
                 |where  a.main_category in ('kids','sports','mv')
                     """.stripMargin
