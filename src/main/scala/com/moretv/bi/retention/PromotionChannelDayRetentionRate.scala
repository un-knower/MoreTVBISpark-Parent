...
object PromotionChannelDayRetentionRate extends BaseClass {
        ...
        for (i <- 0 until numOfDays) {
          ...
          for (j <- 0 until needToCalc.length) {
            ...
            DataIO.getDataFrameOps.getDF(sc, p.paramMap, DBSNAPSHOT, LogTypes.MORETV_MTV_ACCOUNT,dateEN).
              select("user_id","promotion_channel","openTime").
              filter(s"openTime between '$date2 00:00:00' and '$date2 23:59:59' and promotion_channel in ('boshilian')").
              selectExpr("user_id as userId","promotion_channel as promotionChannel").registerTempTable("new_user")
  ...
  def insertSQL(date: String, typeInfo: String, count: Int, retention: Double, stmt: Statement) = {
    val sql = s"INSERT INTO medusa.`promotion_channel_day_retention_rate` (day,type_info, new_num, d1) VALUES('$date','$typeInfo', $count, $retention)"
    stmt.executeUpdate(sql)
  }

  def updateSQL(num: String, typeInfo: String, retention: Double, date: String, stmt: Statement) = {
    val sql = s"UPDATE medusa.`promotion_channel_day_retention_rate` SET $num = $retention WHERE day = '$date' and type_info ='$typeInfo'"
    stmt.executeUpdate(sql)
  }

  def deleteSQL(date: String, stmt: Statement) = {
    val sql = s"DELETE FROM medusa.`promotion_channel_day_retention_rate` WHERE day = ${date}"
    stmt.execute(sql)
  }
}