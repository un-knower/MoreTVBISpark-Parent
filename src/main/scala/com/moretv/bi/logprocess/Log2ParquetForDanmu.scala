...
object Log2ParquetForDanmu extends BaseClass{
    ...
    ParamsParseUtil.parse(args) match {
      case Some(p) => {
        ...
        (0 until p.numOfDays).foreach(i => {
          ...
          val inputPath = s"/log/danmu/rawlog/danmulog.access.log.$inputDate-*"
          val outputPath = s"/log/danmu/parquet/$inputDate/"
