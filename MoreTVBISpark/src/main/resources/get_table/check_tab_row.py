...
def get_table_row(data):
    ...
    for check_table in all_table:
        ...
        try:
            ...
            if check_data:
                check_result[check_table] = check_data[0][0]
        except:
...
def main():
    ...
    section = sys.argv[2]
    #section='whaley_bi'
    #filename='mysql-table.ini'
    check_data = get_config(filename, section)
    output=section+'.log'
    out_filename = os.path.join(os.path.abspath('.'), output)
    data = get_table_row(check_data)
    write_result(out_filename, data)
    print "END"
