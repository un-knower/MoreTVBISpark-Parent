...
object Diandilamei extends BaseClass{
  def main(args: Array[String]) {
    ModuleClass.executor(Diandilamei,args)
  }
    ...
    ParamsParseUtil.parse(args) match {
      case Some(p) => {
        ...
        //  val date = DateFormatUtils.readFormat.format(cal.getTime)
          val pageviewPath = "/log/activity/parquet/"+p.startDate+"/pageview"
          val watchvideoPath = "/log/activity/parquet/"+p.startDate+"/operation"
          val pageviewRdd = sqlContext.read.load(pageviewPath).filter("activityId = 'i67oe523m7p8'").select("userId").cache()
          val watchvideoRdd = sqlContext.read.load(watchvideoPath).
            filter("activityId = 'i67oe523m7p8' and event = 'watchvideo'").select("userId").cache()
