 ...
 * 播放率对比：播放率=播放人数/活跃人数
 */
object medusaAndMoretvLauncherClickInfo extends SparkSetting{
      ...
      case Some(p) => {
        ...
        (0 until p.numOfDays).foreach(i=>{
          ...
          val sqlSumSparkMedusa = "" +
            "select a.apkVersion,a.buildDate,launcherAccessAreaParser(a.accessArea,'medusa'),count(a.userId),count" +
            "(distinct a.userId) from medusa_homeaccess_log as a where a.userId not like '999999999999%' and " +
            "a.accessArea'recommendation' and a.event='click' and " +
            "group by a.apkVersion,a.buildDate,launcherAccessAreaParser(a.accessArea,'medusa')"
