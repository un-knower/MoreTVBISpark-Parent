...
object DataAnalyticsAppUtilizeDurationDistribution extends BaseClass {

  private val tableName = "data_analytic_app_utilize_duration_distribution"
      ...
      case Some(p) => {
        ...
        (0 until p.numOfDays).foreach(i => {
          ...
          val insertSql = s"insert into $tableName(day,duration,play_num,total_num) values(?,?,?,?)"
            DataIO.getDataFrameOps.getDF(sc, p.paramMap, MEDUSA, LogTypes.EXIT,date).
