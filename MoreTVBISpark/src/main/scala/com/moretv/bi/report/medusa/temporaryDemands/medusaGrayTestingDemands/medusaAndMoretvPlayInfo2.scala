 ...
 * 播放率对比：播放率=播放人数/活跃人数
 */
object medusaAndMoretvPlayInfo2 extends SparkSetting{
      ...
      case Some(p) => {
        ...
        (0 until p.numOfDays).foreach(i=>{
          ...
          val medusaEnterUserRdd = sqlContext.sql("select count(userId),count(distinct userId) from " +
            "medusa_enter_log where userId not like '999999999999%' and apkVersion='3.0.5' and buildDate = '20160518'")
          ...
          val medusaPlayUserRdd = sqlContext.sql(
            "select count(userId),count(distinct userId),sum(duration)" +
              " from medusa_play_log where userId not like '99999999999999%' and apkVersion='3.0.5' and buildDate = '20160518' and" +
          ...
          */
          ...
          val medusaHistoryAndCollectPlayUserRdd = sqlContext.sql(
            "select launcherAccessLocationFromPath,count(a.userId),count(distinct a.userId),sum(a.duration)" +
              " from medusa_moretv_play_log as a where a.userId not " +
              "like '99999999999999%' and a.apkVersion='3.0.5' and a.buildDate = '20160518'  and a.duration<=10800 and a" +
          ...
          val medusaEachContentTypePlayUserRdd = sqlContext.sql(
            "select contentType,count(userId),count(distinct userId),sum(duration)" +
              ...
              "'99999999999999%' and apkVersion='3.0.5' and buildDate = '20160518' and duration<=10800 and duration>=0" +
          ...
          /*
          util.insert(sqlInsert,insertDate,"3.0.5",new JLong(medusaPlayUserRdd(0)._1),new JLong(medusaPlayUserRdd
