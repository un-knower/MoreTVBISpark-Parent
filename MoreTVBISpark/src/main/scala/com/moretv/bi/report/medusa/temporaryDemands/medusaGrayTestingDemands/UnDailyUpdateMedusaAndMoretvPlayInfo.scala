 ...
 * 播放率对比：播放率=播放人数/活跃人数
 */
object UnDailyUpdateMedusaAndMoretvPlayInfo extends SparkSetting{
    ...
    ParamsParseUtil.parse(args) match {
      case Some(p) => {
        ...
        (0 until p.numOfDays).foreach(i=>{
          ...
          val medusaEnterUserRdd = sqlContext.sql("select count(a.userId),count(distinct a.userId) from " +
            "medusa_enter_log as a join medusa_un_daily_update_log as b on a.userId=b.userId where a.userId not like " +
            "'999999999999%' and a.apkVersion='3.0.6'")
            .map(e=>(e.getLong(0),e.getLong(1))).map(e=>(e._1,e._2)).collect()

          val medusaPlayUserRdd = sqlContext.sql(
            "select count(a.userId),count(distinct a.userId),sum(a.duration)" +
              " from medusa_play_log as a join medusa_un_daily_update_log as b on a.userId=b.userId where a.userId not " +
              "like '99999999999999%' and a.apkVersion='3.0.6' and a.duration<=10800 and a.duration>=0 and a.event='playview'")
            .map(e=>(e.getLong(0),e.getLong(1),e.getLong(2))).map(e=>(e._1,e._2,e._3)).collect()


          val medusaEachAreaPlayUserRdd = sqlContext.sql(
            "select a.launcherAreaFromPath,count(a.userId),count(distinct a.userId),sum(a.duration)" +
              " from medusa_moretv_play_log as a join medusa_un_daily_update_log as b on a.userId=b.userId where a.userId " +
              "not like '99999999999999%' and a.apkVersion='3.0.6' and a.duration<=10800 and a" +
              ".duration>=0 and a.event ='playview' and a.launcherAreaFromPath in ('classification'," +
              "'recommendation','my_tv','foundation') group by a.launcherAreaFromPath")
          ...
          val medusaClassificationPlayUserRdd = sqlContext.sql(
            "select 'classificationAndMytv' as launcherAreaFromPath,count(a.userId),count(distinct a.userId),sum(a" +
              ...
              "and a.launcherAccessLocationFromPath not in ('account','history','collect') ")
            .map(e=>(e.getString(0),e.getLong(1),e.getLong(2),e
            .getLong(3))).map(e=>(e._1,e._2,e._3,e._4)).collect()
          ...
          val medusaHistoryAndCollectPlayUserRdd = sqlContext.sql(
            "select 'historyAndcollect' as launcherAccessLocationFromPath,count(a.userId),count(distinct a.userId),sum(a" +
              ".duration) from medusa_moretv_play_log as a join medusa_un_daily_update_log as b on a.userId=b.userId where" +
              " a" +
              ".userId not like '99999999999999%' and a.apkVersion='3.0.6'  and a.duration<=10800 and a" +
          ...
          val medusaHistoryCollectPlayUserRdd = sqlContext.sql(
            "select a.launcherAccessLocationFromPath,count(a.userId),count(distinct a.userId),sum(a" +
              ".duration) from medusa_moretv_play_log as a join medusa_un_daily_update_log as b on a.userId=b.userId where" +
              " a" +
              ".userId not like '99999999999999%' and a.apkVersion='3.0.6'  and a.duration<=10800 and a" +
              ".duration>=0 and a.event ='playview' and a.launcherAccessLocationFromPath in ('history'," +
              "'collect') group by a.launcherAccessLocationFromPath")
            .map(e=>(e.getString(0),e.getLong(1),e.getLong(2),e
            .getLong(3))).map(e=>(e._1,e._2,e._3,e._4)).collect()
          ...
          val medusaEachContentTypePlayUserRdd = sqlContext.sql(
            "select a.contentType,count(a.userId),count(distinct a.userId),sum(a.duration)" +
              " from medusa_play_log as a join medusa_un_daily_update_log as b on a.userId=b.userId where a.userId not " +
              "like '99999999999999%' and a.apkVersion='3.0.6' and a.duration<=10800 and a.duration>=0" +
              " and a.event='playview' group by a.contentType")
            .map(e=>(e.getString(0),e.getLong(1),e.getLong(2),e.getLong(3))).map(e=>(e._1,e._2,e._3,e._4)).collect()

          val moretvEnterUserRdd = sqlContext.sql("select count(a.userId),count(distinct a.userId) from " +
            "moretv_enter_log as a join moretv_un_daily_update_log as b on a.userId=b.userId where a.userId not like " +
            "'999999999999%' and a.apkVersion='2.6.7'")
            .map(e=>(e.getLong(0),e.getLong(1))).map(e=>(e._1,e._2)).collect()
          val moretvPlayUserRdd = sqlContext.sql("select count(a.userId),count(distinct a.userId),sum(a.duration) from " +
            "moretv_play_log as a join moretv_un_daily_update_log as b on a.userId=b.userId where a.userId not like " +
            "'999999999999%' and a.apkVersion='2.6.7' and a.event='playview' and " +
            "a.duration<=10800 and a.duration>=0")
            .map(e=>(e.getLong(0),e.getLong(1),e.getLong(2))).map(e=>(e._1,e._2,e._3)).collect()




          val sqlInsert = "insert into medusa_gray_testing_play_info_undailyupdate_each_day(date,apk_version,play_num," +
            "play_user," +
            "play_duration,enter_num,enter_user) values (?,?,?,?,?,?,?)"
          val sqlInsert1 = "insert into medusa_gray_testing_each_contentType_play_undailyupdate_each_day(date,apk_version," +
            "contentType,play_num, play_user,play_duration) values (?,?,?,?,?,?)"
          val sqlInsert2 = "insert into medusa_gray_testing_each_area_play_info_undailyupdate_each_day(date,apk_version,area,play_num, " +
