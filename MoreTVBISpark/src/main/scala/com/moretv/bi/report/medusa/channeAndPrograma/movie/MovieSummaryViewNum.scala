 ...
 */
object MovieSummaryViewNum extends BaseClass{
  def main(args: Array[String]) {
    ModuleClass.executor(MovieSummaryViewNum,args)
  }
  override def execute(args: Array[String]) {
    ...
    val dbMode = "medusa"
    val tableName = "medusa_programa_movie_summary_view_user_num"
    val sqlSpark = "select count(userId),count(distinct userId) from log_data where event='view' and contentType='movie'"
    val sqlInsert = "insert into medusa_programa_movie_summary_view_user_num(day,view_num,user_num) values (?,?,?)"
    ...
    db.insert(insertConfigSql,"MovieSummaryViewNum",logType,jsonObj.toString(),"夏俊","2016-05-03","medusa","CountStatistic",
      "该应用用于统计频道与栏目编排中详情页的浏览人数！")
  }
}
