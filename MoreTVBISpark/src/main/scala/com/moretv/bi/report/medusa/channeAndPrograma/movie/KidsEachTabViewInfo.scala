    ...
    ParamsParseUtil.parse(args) match {
      case Some(p) => {
        ...
        (0 until p.numOfDays).foreach(i=>{
          ...
          /*数据库操作*/
          if(p.deleteOld){
            val deleteSql = "delete from medusa_channel_eachtab_view_kids_info where day = ?"
            util.delete(deleteSql,insertDate)
          }
          val sqlInsert = "insert into medusa_channel_eachtab_view_kids_info(day,channelname,tabname,view_num,view_user) " +
            "values (?,?,?,?,?)"
