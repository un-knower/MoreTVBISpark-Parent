       ...
       case Some(p)=>{
         ...
         (0 until p.numOfDays).foreach(i=>{
          ...
          /* val logDir1 = s"$medusaDir/$inputDate/$logType"
           ...
           val medusa_input_dir= DataIO.getDataFrameOps.getPath(MEDUSA,LogTypes.HOMEACCESS,inputDate)
           val moretv_input_dir= DataIO.getDataFrameOps.getPath(MORETV,LogTypes.HOMEACCESS,inputDate)
           val outputPath= DataIO.getDataFrameOps.getPath(MERGER,LogTypes.HOMEACCESS,inputDate)
           ...
           if(medusaFlag && moretvFlag){
            ...
            /* val medusaDf = sqlContext.read.parquet(logDir1)
             ...
             val medusaDf = DataIO.getDataFrameOps.getDF(sqlContext,p.paramMap,MEDUSA,LogTypes.HOMEACCESS,inputDate)
             val moretvDf = DataIO.getDataFrameOps.getDF(sqlContext,p.paramMap,MORETV,LogTypes.HOMEACCESS,inputDate)
