...
import com.moretv.bi.report.medusa.medusaGrayTestingStatistic.CrashAndLoginInfoByVersionAndProduct._
 ...
 */
object CrashAndLoginInfoByVersionAndProductCrashType extends BaseClass{
    ...
    ParamsParseUtil.parse(args) match {
      case Some(p) => {
       ...
       /* val logType = "homeview"
        ...
        val sqlInsert = "insert into medusa_crash_login_info_by_product_version_special_crash_key(day,apk_version," +
          "productModel," +
          "login_num,login_user,crash_num,crash_user) values (?,?,?,?,?,?,?)"
        val sqlDelete = "delete from medusa_crash_login_info_by_product_version_special_crash_key where day = ?"
        ...
        (0 until p.numOfDays).foreach(i=>{
          ...
          // 获取每天crash的人数与次数
          val minIdNum = util.selectOne(s"select min(id) from medusa_crash_product_version_special_crash_key_num where " +
            s"day='$date'")(0).toString.toLong
          val maxIdNum = util.selectOne(s"select max(id) from medusa_crash_product_version_special_crash_key_num where " +
            s"day='$date'")(0).toString.toLong
          val minIdUser = util.selectOne(s"select min(id) from medusa_crash_product_version_special_crash_key_user where " +
            s"day='$date'")(0).toString.toLong
          val maxIdUser = util.selectOne(s"select max(id) from medusa_crash_product_version_special_crash_key_user where " +
            s"day='$date'")(0).toString.toLong
        ...
        /*  val crashNumRdd = new JdbcRDD(sc,
            ...
            "select day,app_version_name,product_code,total_number from medusa_crash_product_version_special_crash_key_num" +
              " where id >= " +
              "? and" +
              " id <= ?",
            minIdNum,
          ...
          val crashNumRddSql ="select day,app_version_name,product_code,total_number from medusa_crash_product_version_special_crash_key_num" +
          " where id >= " +
          "? and" +
          " id <= ?"
          val crashNumRdd = MySqlOps.
            getJdbcRDD(sc,crashNumRddSql,"medusa_crash_product_version_special_crash_key_num",r=>(r.getString(1),r.getString(2),r.getString(3),r.getLong(4)),driver,url,user,password,(minIdNum ,maxIdNum ),numOfPartition)
         ...
         /* val crashUserNum = new JdbcRDD(sc,
            ...
            "select day,app_version_name,product_code,total_user from medusa_crash_product_version_special_crash_key_user " +
              "where id >= ? and" +
              " id <= ?",
            minIdUser,
            maxIdUser,
          ...
          val  crashUserNumSql="select day,app_version_name,product_code,total_user from medusa_crash_product_version_special_crash_key_user " +
            "where id >= ? and" +
            " id <= ?"
          val crashUserNum = MySqlOps.
            getJdbcRDD(sc,crashUserNumSql,"medusa_crash_product_version_special_crash_key_user",r=>(r.getString(1),r.getString(2),r.getString(3),r.getLong(4)),driver,url,user,password,(minIdUser,maxIdUser),numOfPartition)
