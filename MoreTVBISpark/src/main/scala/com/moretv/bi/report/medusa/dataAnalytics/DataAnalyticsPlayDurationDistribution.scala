    ...
    ParamsParseUtil.parse(args) match {
      case Some(p) => {
        ...
        (0 until p.numOfDays).foreach(i => {
          ...
          println("result0.collect().size:"+result0.collect().size)
           /*
          ...
          //for test
          val result1=sqlContext.sql("select count(1) from log_play")
          println("result1.collect().size:"+result1.collect().size)
          ...
          //for test
          val result2=sqlContext.sql("select count(1) from log_play_distribution")
          println("result2.collect().size:"+result2.collect().size)

          /*统计在不同观看时常纬度上的用户数量分布，附带特定duration_sum，特定playUser的总用户数信息*/
          sqlStr="""
            |select a.contentType,a.version,a.duration_sum,a.playUser,b.totalUser
