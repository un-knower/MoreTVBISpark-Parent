  ...
  override def execute(args: Array[String]) {
    ...
    ParamsParseUtil.parse(args) match {
      case Some(p) =>{
        ...
        val evaluate = df.filter("event='evaluate'").select("date","contentType","userId").map(e =>(e.getString(0),e.getString(1),e.getString(2))).
                        map(e=>(getKeys(e._1,e._2,"evaluate"),e._3))
        val resultRDD = df.filter("event='evaluate'").select("date","contentType","action","userId").map(e =>(e.getString(0),e.getString(1),e.getString(2),e.getString(3))).
                           map(e=>(getKeys(e._1,e._2,e._3),e._4)).union(evaluate).persist(StorageLevel.MEMORY_AND_DISK)
