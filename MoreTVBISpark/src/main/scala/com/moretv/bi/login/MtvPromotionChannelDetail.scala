  ...
  override def execute(args: Array[String]) {
    ...
    ParamsParseUtil.parse(args) match {
      case Some(p) => {
        ...
        val logRddMoretv = DataIO.getDataFrameOps.getDF(sc, p.paramMap, LOGINLOG, LogTypes.LOGINLOG)
        ...
        val logRddKids = DataIO.getDataFrameOps.getDF(sc, p.paramMap, MTVKIDSLOGIN, LogTypes.LOGINLOG)
          .select("promotionChannel", "mac")
          .map(row => {
            val promotionChannel = row.getString(0)
            if (promotionChannel != null) {
