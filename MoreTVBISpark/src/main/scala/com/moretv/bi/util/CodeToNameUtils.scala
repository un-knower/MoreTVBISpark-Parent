...
object CodeToNameUtils {
  ...
  val sid2SubjectNameMapSql: String = "SELECT code,name FROM tvservice.mtv_subject WHERE code IS NOT NULL and name is not null"
  val subjectName2CodeMapSql: String = "SELECT name,code FROM tvservice.mtv_subject WHERE name IS NOT NULL AND code IS NOT NULL and name is not null"
  ...
  /**
    ...
    * @param map
    */
  def initSidMap(url: String, sql: String, map: Map[String, String]): Map[String, String] = {
    try {
