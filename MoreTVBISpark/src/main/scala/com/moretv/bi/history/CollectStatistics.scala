    ...
    ParamsParseUtil.parse(args) match {
      case Some(p) => {
        ...
        (0 until p.numOfDays).foreach(i => {
          ...
          val dfDetail = DataIO.getDataFrameOps.getDF(sc, p.paramMap, MORETV, LogTypes.DETAIL, date)
            .filter("path = 'home-history-collect'")
            .select("userId", "contentType")
            .cache()

          val dfPlayview = DataIO.getDataFrameOps.getDF(sc, p.paramMap, MORETV, LogTypes.PLAYVIEW, date)
            .filter("path = 'home-history-collect'")
            .select("userId", "contentType")
            .cache()
