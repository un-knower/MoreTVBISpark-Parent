  ...
  override def execute(args: Array[String]) {
    ...
    ParamsParseUtil.parse(args) match {
      case Some(p) => {
        ...
        val df = DataIO.getDataFrameOps.getDF(sc, p.paramMap, MORETV, LogTypes.PLAYVIEW).unionAll(
          DataIO.getDataFrameOps.getDF(sc, p.paramMap, MORETV, LogTypes.DETAIL)
        )

        val playRDD = df.filter("apkVersion > '2.4.7' and logType='playview' and event ='playview'")
          .select("date", "path", "contentType", "userId")
        ...
        val detailRDD = df.filter("apkVersion > '2.4.7' and logType='detail'")
          .select("date", "path", "contentType", "userId")
          .map(e => (e.getString(0), e.getString(1), e.getString(2), e.getString(3)))
          .filter(e => judgePath(e._2)).map(e => (getKeys(e._1, e._2, e._3, "detail", ""), e._4))
          .filter(e => e._1 != null).persist(StorageLevel.MEMORY_AND_DISK)
