 ...
 */
object IndexCollectTop500Statistics extends BaseClass with QueryMaxAndMinIDUtil with DateUtil{
  val reg ="log=play-001-playview-[\\w\\.]+_\\d+\\.\\d+\\.\\d+-([a-f0-9]{32}-\\d*-\\d*|[a-f0-9]{32}-\\d*|[a-f0-9]{32})-home-history-collect-(movie|tv|zongyi|kids|comic|jilu)-([A-Za-z0-9]{12})-".r

  def main(args: Array[String]) {
    config.setAppName("IndexCollectTop500Statistics")
    ModuleClass.executor(IndexCollectTop500Statistics,args)
  }
  override def execute(args: Array[String]) {
    ...
    val doc = JsonDocument.create(timeFlage+"_IndexCollectTagTop500",content)
    bucket.upsert(doc)
    cluster.disconnect()
