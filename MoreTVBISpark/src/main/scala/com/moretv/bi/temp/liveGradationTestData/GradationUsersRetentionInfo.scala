 ...
 */
object GradationUsersRetentionInfo extends SparkSetting {
  private val tableName = "medusa_live_gradation_test_auc_retention_info"
      ...
      case Some(p) => {
        ...
        (0 until p.numOfDays).foreach(i => {
          ...
          (0 until needToCalc.length).foreach(j => {
            ...
            sqlContext.read.parquet(inputDir1).select("userId").registerTempTable("log_all1")
            val userRdd1 = sqlContext.sql(
              """select distinct a.userId
                |from log_all1 as a join log_gradation as b on a.userId=b.userId
                |""".stripMargin).
              map(e => e.getString(0))
