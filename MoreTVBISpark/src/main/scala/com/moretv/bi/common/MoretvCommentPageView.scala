...
/**
  ...
  */
object MoretvCommentPageView extends BaseClass {
  ...
  override def execute(args: Array[String]) {
    ...
    ParamsParseUtil.parse(args) match {
      case Some(p) => {
        val cacheValue = DataIO.getDataFrameOps.getDF(sc, p.paramMap, MORETV, LogTypes.PAGEVIEW)
          .filter("event='view'").select("date", "userId")
        ...
        if (p.deleteOld) {
          ...
          val oldSql = s"delete from moretvCommentPageView where day = '$date'"
          dbUtil.delete(oldSql)
        }
        userNumValue.foreach(
          x => {
            sql = "insert into moretvCommentPageView(day,user_num,access_num) values(?,?,?)"
