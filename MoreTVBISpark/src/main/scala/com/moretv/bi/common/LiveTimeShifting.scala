  ...
  override def execute(args: Array[String]) {
    ...
    ParamsParseUtil.parse(args) match {
      case Some(p) => {
        ...
        val cacheValue = DataIO.getDataFrameOps.getDF(sc, p.paramMap, MORETV, LogTypes.OPERATION_ST)
          .filter("event='view'").select("date", "userId").map(e => (e.getString(0), e.getString(1))).persist()
        /** 计算人数和次数 */
        ...
        if (p.deleteOld) {
          ...
          val oldSql = s"delete from liveTimeShifting where day = '$date'"
          dbUtil.delete(oldSql)
        }
        userNumValue.foreach(
          x => {
            sql = "insert into liveTimeShifting(day,user_num,access_num) values(?,?,?)"
