  ...
  override def execute(args: Array[String]) {
    ...
    ParamsParseUtil.parse(args) match {
      case Some(p) =>{
        ...
        val resultRDD = df.filter("event = 'exit' and duration < 72000 and duration >0 ").
          select("date","path","userId","duration").
          map(e =>(e.getString(0),e.getString(1),e.getString(2),e.getInt(3))).filter(e =>judgePath(e._2)).
