  ...
  override def execute(args: Array[String]) {
    ...
    ParamsParseUtil.parse(args) match {
      case Some(p) => {
        ...
        val programMap = DataIO.getDataFrameOps.getDF(sc,p.paramMap,MORETV,LogTypes.OPERATION_ACW,p.startDate).filter("event='addtag'").select("accountId").map(e=>e.getInt(0)+"").filter(_ != 0).distinct()
        val sqlMinMaxId = "select min(id),max(id) from addTagTotalAccountId"
        val sqlData = "SELECT accountId FROM `addTagTotalAccountId` WHERE ID >= ? AND ID <= ?"
