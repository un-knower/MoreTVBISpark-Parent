 ...
 */
object Third_path_pv_uv extends BaseClass with DateUtil{
  def main(args: Array[String]): Unit = {
    config.setAppName("Third_path_pv_uv")
  ...
  override def execute(args: Array[String]) {
    ...
    ParamsParseUtil.parse(args) match {
      case Some(p) => {
        val df = DataIO.getDataFrameOps.getDF(sc,p.paramMap,MORETV,LogTypes.DETAIL)
        ...
        val playRDD = allRDD.union(kidsRDD).persist(StorageLevel.MEMORY_AND_DISK)
        ...
        if (p.deleteOld) {
          ...
          val oldSql = s"delete from third_path_pv_uv where day = '$date'"
          util.delete(oldSql)
        }
        //insert new data
        val sql = "INSERT INTO third_path_pv_uv(year,month,day,weekstart_end,channel,path,uv_num,pv_num) VALUES(?,?,?,?,?,?,?,?)"
        userNum.foreach(x =>{
  ...
  def getKeys(date:String, pathPar:String, flag:String)={
    ...
    val reg =if(flag == "All") "home-(movie|tv|zongyi|comic|kids|jilu|mv|hot|xiqu|kids_home)-(\\w+)".r  else "home-(kids_home)-kids_seecartoon-(\\w+)".r
