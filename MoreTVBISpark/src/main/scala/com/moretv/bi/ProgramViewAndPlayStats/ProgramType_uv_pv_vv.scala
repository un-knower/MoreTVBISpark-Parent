  ...
  override def execute(args: Array[String]) {
    ...
    ParamsParseUtil.parse(args) match {
      case Some(p) => {
        ...
        playRDD.unpersist()
        val detailRDD = DataIO.getDataFrameOps.getDF(sc,p.paramMap,MORETV,LogTypes.DETAIL).select("date", "contentType", "userId").
        ...
        detailRDD.unpersist()
        val liveRDD = DataIO.getDataFrameOps.getDF(sc,p.paramMap,MORETV,LogTypes.LIVE).select("date", "userId").
