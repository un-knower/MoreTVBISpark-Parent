  ...
  override def execute(args: Array[String]) {
    ...
    ParamsParseUtil.parse(args) match {
      case Some(p) => {
        ...
        val playRDD = df1.filter("logType='playview' and path like '%kids_home%'").select("date", "path", "userId").map(e => (e.getString(0), e.getString(1), e.getString(2))).
        ...
        val detailRDD = df2.filter("logType='detail' and path like '%kids_home%'").select("date", "path", "userId").map(e => (e.getString(0), e.getString(1), e.getString(2))).
