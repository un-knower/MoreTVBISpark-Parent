    ...
    ParamsParseUtil.parse(args) match {
      case Some(p) => {
        ...
        val playRDD = DataIO.getDataFrameOps.getDF(sc, p.paramMap, MORETV, LogTypes.PLAYVIEW)
          .filter("logType='playview'").select("date", "path", "userId")
          .map(e => (e.getString(0), e.getString(1), e.getString(2)))
          .filter(e => judgePath(e._2, "play")).map(e => (getKeys(e._1), e._3))

        val detailRDD = DataIO.getDataFrameOps.getDF(sc, p.paramMap, MORETV, LogTypes.LIVE)
          .select("date", "path", "userId")
          .map(e => (e.getString(0), e.getString(1), e.getString(2)))
          .filter(e => judgePath(e._2, "live"))
          .map(e => (getKeys(e._1), e._3)).union(playRDD).persist(StorageLevel.MEMORY_AND_DISK)
