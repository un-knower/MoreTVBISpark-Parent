  ...
  override def execute(args: Array[String]) {
    ...
    ParamsParseUtil.parse(args) match {
      case Some(p) => {
        ...
        val playRDD = df.filter("logType='playview' and apkVersion >'2.4.5' and duration < 3600 and duration > 0").select("date","path","duration").map(e => (e.getString(0), e.getString(1), e.getInt(2))).
            filter(e =>judgePath(e._2,"play")).map(e => (getKeys(e._1, e._2), e._3)).persist(StorageLevel.MEMORY_AND_DISK)
        val palyDurationRDD = playRDD.map(e =>((e._1._1,e._1._2,e._1._3,"total",e._1._5),e._2)).union(playRDD).reduceByKey((x,y)=>x+y).collect().toMap


        val liveRDD = df.filter("logType='live' and liveType='live' and duration < 3600 and duration > 0 ").
        ...
        val liveDurationRDD = liveRDD.map(e =>((e._1._1,e._1._2,e._1._3,"total",e._1._5),e._2)).union(liveRDD).reduceByKey((x,y)=>x+y).collect().toMap


        val pastRDD = df.filter("logType='live' and liveType='past' and duration < 3600 and duration > 0 ").
          select("date", "path","duration").map(e => (e.getString(0), e.getString(1), e.getInt(2))).
            filter(e =>judgePath(e._2,"live")).map(e => (getKeys(e._1, e._2), e._3)).persist(StorageLevel.MEMORY_AND_DISK)
        val pastDurationRDD = pastRDD.map(e =>((e._1._1,e._1._2,e._1._3,"total",e._1._5),e._2)).union(pastRDD).reduceByKey((x,y)=>x+y).collect().toMap
