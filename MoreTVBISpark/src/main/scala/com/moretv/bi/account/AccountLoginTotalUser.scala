  ...
  override def execute(args: Array[String]) {
    ParamsParseUtil.parse(args) match {
      case Some(p) =>{
        ...
        val resultRDD = df.filter("event='login'").select("date","userId","path").map(e =>(e.getString(0),e.getString(1),e.getString(2))).filter(e=>(e._3=="setting" || e._3=="history" || e._3=="addtag" || e._3=="comment")).
