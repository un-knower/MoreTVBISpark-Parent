 ...
 */
object TempDetailLogMerger extends SparkSetting{
    ...
    ParamsParseUtil.parse(args) match {
      case Some(p)=>{
        ...
        (0 until p.numOfDays).foreach(i=>{
          ...
          val outputPath = s"/log/temp/medusaAndMoretvMerger/parquet/$inputDate/$logType"
          ...
          if(medusaFlag && moretvFlag){
            ...
            val sqlSelectMoretv = s"select $moretvColNames, "+
              ...
              s"'moretv' as flag " +
              s" from log_data_2 where userId not in ($moretvUserId)"
